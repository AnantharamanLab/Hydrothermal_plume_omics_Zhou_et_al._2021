...
# VIVID: VIrus to VIrus Designation
# VIGOR: VIral Grouping and ORdering
# VITAL: VIral TAxonomic cLassification
# PIVOTAL: Phage and VIrus TAxonomic cLassification
# VALOR: VirAL ORdering
# v0.9.0

# Usage: $ python3 VIVID.py -i <input_file> -t <threads>
...
############################### Set Arguments  #################################
VIVID_path = str(os.path.dirname(os.path.abspath(__file__)))
VIVID = argparse.ArgumentParser(description='Estimates phage taxonomy.')
VIVID.add_argument('--version', action='version', version='VIVID v0.9.0')

VIVID.add_argument('-i', type=str, nargs=1, required=True, help='input protein fasta file. Must be in format "name_#" with no spaces in protein name.')
VIVID.add_argument('-t', type=str, nargs=1, default='1', help='number of threads [default=1]')
VIVID.add_argument('-f', type=str, nargs=1, default='prot', choices=['prot','nucl'], help='format of input [default="prot"]')
VIVID.add_argument('-d', type=str, nargs=1, default=str(VIVID_path) + '/VIVID_database.dmnd', help='path to diamond database')
VIVID.add_argument('-v', type=str, nargs=1, default=str(VIVID_path) + '/VIVID_virus_taxonomy.tsv', help='path to reference taxonomy file')

args = VIVID.parse_args()
input = str(args.i[0])
database = args.d
taxonomy = args.v
threads = args.t[0]
...
if format == "prot" or format == "p":
    ...
    subprocess.run("diamond blastp -d " + str(database) + " -q " + str(base)+'.temp.fasta' + " -o " + str(base) + ".VIVID.diamond.out --max-target-seqs 3 -e 1e-5 --query-cover 10 --subject-cover 10 --threads " + str(threads) + " --quiet -f 6 qseqid sseqid pident evalue bitscore", shell=True)
...
if format == "nucl" or format == "n":
    ...
    subprocess.run("diamond blastp -d " + str(database) + " -q " + str(base)+'.temp.faa' + " -o " + str(base) + ".VIVID.diamond.out --max-target-seqs 3 -e 1e-5 --query-cover 10 --subject-cover 10 --threads " + str(threads) + " --quiet -f 6 qseqid sseqid pident evalue bitscore", shell=True)

subprocess.run('cat ' + str(base) + '.VIVID.diamond.out | sed "s/~~/ /g" 1> ' + str(base) + '.VIVID.diamond 2>/dev/null', shell=True)
subprocess.run('rm ' + str(base) + '.VIVID.diamond.out 2>/dev/null', shell=True)
...
with open(str(base) + ".VIVID.diamond", "r") as diamond:
    with open(str(base) + ".VIVID.virus-topreps.tsv", "w") as outfile:
        with open(str(base) + ".VIVID.virus-tophits.tsv", "w") as temp:
...
written = []
with open(str(base) + ".VIVID.virus-topreps.tsv", "r") as taxfile:
    with open(str(base) + ".VIVID.virus-taxonomy.tsv", "w") as outfile:
        with open(str(base) + ".VIVID.virus-tophits.tsv", "r") as helper:
...
with open(str(base) + ".VIVID.diamond", "r") as diamond:
    ...
    write = drop.to_csv(str(base) + ".VIVID.diamond-parsed.tsv", index=False, sep="\t")

with open(str(base) + ".VIVID.diamond-parsed.tsv", "r") as diamond:
    with open(str(base) + ".VIVID.protein-taxonomy.tsv", "w") as outfile:
        diamond = diamond.read().replace("\n","\t").split("\t")
        if diamond[-1] == '':
            diamond = diamond[:-1]
        diamond.extend(['\t','\t','\t','\t','\t'])
...
#subprocess.run("rm " + str(base) + ".VIVID.virus-tophits.tsv", shell=True)

logging.basicConfig(filename='VIVID_log_' + str(base) + '.log', level=logging.INFO, format='%(message)s')
...
logging.info("Program:  VIVID v0.9.0")
logging.info('                                                               ##')
logging.info('                                                             ##  ##')
logging.info('                                                           ##      ##')
logging.info('######   ##  ##     ##     #######   ######    #####       ##      ##')
...
subprocess.run('mkdir VIVID_' + str(base) + '_' + str(date.today()), shell=True)
subprocess.run('mkdir VIVID_' + str(base) + '_' + str(date.today()) + '/analysis_files', shell=True)
subprocess.run('mv VIVID_log_' + str(base) + '.log VIVID_' + str(base) + '_' + str(date.today()), shell=True)
subprocess.run('mv ' + str(base) + '.VIVID.*-taxonomy.tsv VIVID_' + str(base) + '_' + str(date.today()), shell=True)
subprocess.run('mv ' + str(base) + '.VIVID.* VIVID_' + str(base) + '_' + str(date.today()) + '/analysis_files', shell=True)
if format == "nucl" or format == "n":
    subprocess.run('mv ' + str(base) + '.faa VIVID_' + str(base) + '_' + str(date.today()), shell=True)
#
#
#
