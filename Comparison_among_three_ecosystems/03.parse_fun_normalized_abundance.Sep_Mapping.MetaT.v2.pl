...
open INN, "ls /mnt/storage7/zhouzhichao/BinProject/hydrothermal_plume_omics/Sep_Mapping.MetaT/parse_KEGG/MetaG_coverage/*MetaG_coverage.txt | ";
while (<INN>){
	my $file = $_;
	my ($env) = $file =~ /MetaG_coverage\/(.+?)\.MetaG_coverage\.txt/;
	my %MAG_cov = (); # MetaG => Bin => Percentage * 100
	my @head = ();
	open IN, "$file";
	while (<IN>){
		chomp;
	...
	foreach my $fun (sort keys %Fun_id){
		foreach my $MetaG (sort keys %MetaG_id){
			foreach my $Bin (sort keys %Bin_id){
				if (exists $Fun_result{$Bin}{$fun} && exists $MAG_cov{$MetaG}{$Bin}){
					$Fun2MetaG{$fun}{$MetaG} += $MAG_cov{$MetaG}{$Bin} * $Fun_result{$Bin}{$fun};
				}else{
	...
	open OUT, ">$env.Fun2MetaG_abundance.txt";
