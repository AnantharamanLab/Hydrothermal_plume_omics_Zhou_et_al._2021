...
while (<IN>){
	chomp;
	$cmd_cat .= $genome_dir."/"."*_".$_.".gene ";
}
close IN;
$cmd_cat .= "> Cayman_gene_cat.gene";
system("$cmd_cat");

my $gn = "Cayman_gene_cat"; 
system ("bowtie2-build $gn.gene $gn.gene_scaffold");
open OUT, ">tmp_Cayman_gene_cat_transcriptom_mapping.txt";
foreach my $key (sort keys %map){
        if ($map{$key}[1] eq "cDNA" and $map{$key}[2] eq "SINGLE" and $map{$key}[0] =~ /^Cym/){
			my $cmd = CMD1($gn, $key);
			print OUT $cmd;
		}elsif ($map{$key}[1] eq "cDNA" and $map{$key}[2] eq "PAIRED" and $map{$key}[0] =~ /^Cym/){
			my $cmd = CMD2($gn, $key);
			print OUT $cmd;
		}
}
...
system ("cat tmp_Cayman_gene_cat_transcriptom_mapping.txt | parallel -j 50");
system ("rm tmp_Cayman_gene_cat_transcriptom_mapping.txt");
