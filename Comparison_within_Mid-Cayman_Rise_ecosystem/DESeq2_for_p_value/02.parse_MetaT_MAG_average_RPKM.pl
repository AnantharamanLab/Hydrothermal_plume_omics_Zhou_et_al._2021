...
my %MetaT_RPKM = (); # gene => MetaT => RPKM * 100  
my @head = ();
my %gene_id = ();
my %Bin_id = ();
...
while (<IN>){
	...
	if ($tmp[0] eq "Head"){
		...
		for(my $i=1; $i<=$#head; $i++){
			$MetaT_RPKM{$gene}{$head[$i]} = $tmp[$i] * 100;
		}
	}	
}
close IN;
...
my %Bin2MetaT2 = (); #The new %Bin2MetaT2 that store the average value of all RPKM
