...
my %MetaT2Cov= (); # metaT => bin => metaT read coverage value
my @MetaT_id = (); 
my @head2 = (); #Store the head line
open IN, "Bin2MetaT_abundance.mdf.txt";
while (<IN>){
	chomp;
	if (/^Head/){
		...
		my @tmp = split (/\t/);
		for(my $i=1; $i<=$#tmp; $i++){
			$MetaT2Cov{$head2[$i]}{$tmp[0]} = $tmp[$i];
		}
	}
}
close IN;

my %Enriched_fun = (); #B.Lau.mean => TIGR00014
my %Fun_id_this_time = ();
open IN, "Enriched_functions.metaT.txt";
while (<IN>){
	chomp;
	my @tmp = split (/\t/);
	$Enriched_fun{$tmp[0]} = $tmp[1];
	$Fun_id_this_time{$tmp[1]} = 1;
}
close IN;


my %Enriched_fun_micro_grp = (); #B.Lau.mean => fun_id => microbial group => percentage;
foreach my $metaT (sort keys %Enriched_fun){
	foreach my $fun_id (sort keys %Fun_id_this_time){
		...
		my %Micro_grp_fun_id_cov = (); #the coverage value of fun trait in certain microbial groups
			...
			foreach my $bin (sort keys %MAG_map){
				if ($Fun_result{$fun_id}{$bin}){
					if ($MetaT2Cov{$metaT}{$bin}){
						$total_fun_id_cov += $MetaT2Cov{$metaT}{$bin} * $Fun_result{$fun_id}{$bin};
					}else{
						$total_fun_id_cov += 0;
					}						
				}
				foreach my $micro_grp (sort keys %Microbial_group){
					if ($micro_grp eq $MAG_map{$bin}[0]){
						if ($Fun_result{$fun_id}{$bin}){
							if ($MetaT2Cov{$metaT}{$bin}){
								$Micro_grp_fun_id_cov{$micro_grp} += $MetaT2Cov{$metaT}{$bin} * $Fun_result{$fun_id}{$bin};
							}else{
		...
		foreach my $micro_grp (sort keys %Micro_grp_fun_id_cov){
			if ($Micro_grp_fun_id_cov{$micro_grp}){
				$Enriched_fun_micro_grp{$metaT}{$fun_id}{$micro_grp} = $Micro_grp_fun_id_cov{$micro_grp} / $total_fun_id_cov;
			}else{
				$Enriched_fun_micro_grp{$metaT}{$fun_id}{$micro_grp} = 0;
			}
		}		
	}	
}
...
foreach my $metaT (sort keys %Enriched_fun){
	open OUT, ">Enriched_fun_micro_grp.$metaT.metaT.txt";
	my $row=join("\t", sort keys %Fun_id_this_time);
	print OUT "Head\t$row\n";
	foreach my $tmp1 (sort keys %Microbial_group)
	{
        print OUT $tmp1."\t";
        my @tmp = ();
        foreach my $tmp2 (sort keys %Fun_id_this_time)
        {
                if (exists $Enriched_fun_micro_grp{$metaT}{$tmp2}{$tmp1})
                {
                        push @tmp, $Enriched_fun_micro_grp{$metaT}{$tmp2}{$tmp1};
                }
                else
                {
                        push @tmp,"0"
