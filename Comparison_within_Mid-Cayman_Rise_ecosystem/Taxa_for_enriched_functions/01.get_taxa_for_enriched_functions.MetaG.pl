...
open IN, "MAG_average_coverage.mdf.txt";
while (<IN>){
	chomp;
	if (/^Head/){
...
my %Fun_id_this_time = ();
open IN, "Enriched_functions.txt";
while (<IN>){
	chomp;
	my @tmp = split (/\t/);
...
foreach my $metaG (sort keys %Enriched_fun){
	foreach my $fun_id (sort keys %Fun_id_this_time){
		...
		foreach my $micro_grp (sort keys %Micro_grp_fun_id_cov){
			if ($Micro_grp_fun_id_cov{$micro_grp}){
				$Enriched_fun_micro_grp{$metaG}{$fun_id}{$micro_grp} = $Micro_grp_fun_id_cov{$micro_grp} / $total_fun_id_cov;
			}else{
				$Enriched_fun_micro_grp{$metaG}{$fun_id}{$micro_grp} = 0;
			}
		}		
	}	
}
...
foreach my $metaG (sort keys %Enriched_fun){
	open OUT, ">Enriched_fun_micro_grp.$metaG.metaG.txt";
	...
	foreach my $tmp1 (sort keys %Microbial_group)
	{
        ...
        {
                if (exists $Enriched_fun_micro_grp{$metaG}{$tmp2}{$tmp1})
                {
                        push @tmp, $Enriched_fun_micro_grp{$metaG}{$tmp2}{$tmp1};
                }
                else
                {
                        push @tmp,"0"
